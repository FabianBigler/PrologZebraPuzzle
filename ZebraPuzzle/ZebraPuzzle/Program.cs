...
namespace Zebra
{
    ...
    class ZebraPuzzleConstraints
    {
        ...
        public CspTerm[] TheGreenHouseIsImmediatelyLeftOfTheWhiteHouse()
        {
            var constraints = new List<CspTerm>
            {
                Solver.Equal(0, ColourMatrix[0][(int) HouseColour.White]),
                Solver.Equal(ColourMatrix[1][(int)HouseColour.White],
                    ColourMatrix[0][(int)HouseColour.Green])
            };
        ...
        public CspTerm[] TheManWhoSmokesBlendInHouseNextToHouseWithCats()
        {
            var constraints = new List<CspTerm>
            {
            ...
            Enumerable.Range(1, 3).ToList()
                .ForEach(houseNumber => constraints.Add(
                    Solver.Greater(1, Solver.Sum(
        ...
        public CspTerm TheNorwegianLivesInTheFirstHouse()
        {
            ...
            return Solver.Equal(1, NationalityMatrix[firstHouseNumber][
                (int)Nationality.Norwegian]);
        }
        ...
        public CspTerm[] TheNorwegianLivesNextToTheBlueHouse()
        {
            ...
            Enumerable.Range(1, 3).ToList()
                .ForEach(houseNumber => constraints.Add(
                    Solver.Greater(1, Solver.Sum(
                        NationalityMatrix[houseNumber][
                            (int)Nationality.Norwegian],
                        ColourMatrix[houseNumber][(int)HouseColour.Blue])
                        - Solver.Sum(
                            NationalityMatrix[houseNumber - 1][
                                (int)Nationality.Norwegian],
                            NationalityMatrix[houseNumber + 1][
                                (int)Nationality.Norwegian],
                            ColourMatrix[houseNumber - 1][
                                (int)HouseColour.Blue],
                            ColourMatrix[houseNumber + 1][
                                (int)HouseColour.Blue]))));
        ...
        public CspTerm[] TheyDrinkWaterInHouseNextToHouseWhereSmokeBlend()
        {
            var constraints = new List<CspTerm>
            {
             Solver.Greater(1, Solver.Sum(
        ...
        public CspTerm[] InHouseNextToHouseWhereHaveAHorseSmokeDunhill()
        {
            var constraints = new List<CspTerm>
            {
                Solver.Greater(1, Solver.Sum(
